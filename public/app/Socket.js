...
import ClientPlayer from './World/ClientPlayer.js';
...
export default class SocketConnection {
    ...
    socketReceiveMessages() {
      ...
      this.socket.on('chat message', (msg) => {
        ...
        // add alert icons
        this.messagePanelButton = document.querySelector("#button__messages:not(.active)")
        if(this.messagePanelButton) {
          if(this.id !== msg.id) {
            this.messagePanelButton.classList.add("new_message");
          }
        }

        /// toggle message alert
      ...
      this.socket.on('introduction', (_id, _clientNum, _ids, clients)=>{
        ...
        for(let i = 0; i < _ids.length; i++){
          if(_ids[i] != _id){
            ...
            this.clients[_ids[i]].player = new ClientPlayer(this.clients[_ids[i]].mesh)
    ...
    socketNewExternalUser(){
      ...
      this.socket.on('newUserConnected',  (clientCount, _id, _ids, clients) =>{
        ...
        if(_id != this.id && !alreadyHasUser){
          ...
          this.clients[_id].player = new ClientPlayer(this.clients[_id].mesh)
